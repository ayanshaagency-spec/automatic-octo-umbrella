...
class ApiClient {
  ApiClient({String? baseUrl})
      : baseUrl = baseUrl ??
            const String.fromEnvironment('AYANSHA_API_URL',
  ...
  Future<Map<String, dynamic>> createAppointment({
    ...
    if (response.statusCode != 201) {
      throw _error(response, 'Unable to create appointment');
    }
  ...
  Future<Map<String, dynamic>> createPrescription({
    ...
    final response = await http
        ...
        .timeout(_timeout);
    if (response.statusCode != 201) {
      throw _error(response, 'Unable to create prescription');
    }
    return Map<String, dynamic>.from(jsonDecode(response.body));
  }
  ...
  Future<Map<String, dynamic>> createHealthRecord({
    ...
    final response = await http
        ...
        .timeout(_timeout);
    if (response.statusCode != 201) {
      throw _error(response, 'Unable to create health record');
    }
    return Map<String, dynamic>.from(jsonDecode(response.body));
  }
  ...
  Future<Map<String, dynamic>> getEmergency() async {
    ...
    if (response.statusCode != 200) {
      throw _error(response, 'Unable to load emergency services');
    }
    return Map<String, dynamic>.from(jsonDecode(response.body));
  }
  ...
  Future<List<Map<String, dynamic>>> getNearbyHospitals({
    ...
    final response = await http.get(uri).timeout(_timeout);
    if (response.statusCode != 200) {
      throw _error(response, 'Unable to load nearby hospitals');
    }
    final data = Map<String, dynamic>.from(jsonDecode(response.body));
    return (data['hospitals'] as List? ?? const [])
        .map((e) => Map<String, dynamic>.from(e as Map))
        .toList();
  }
}
