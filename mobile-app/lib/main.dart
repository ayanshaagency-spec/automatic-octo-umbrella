...
class Doctor {
  final int id; final String name; final String specialty; final String experience; final List<String> slots;
...
class Appointment {
  final int? id; final Doctor doctor; final DateTime date; final String slot; final String mode; final String status;
...
class _DoctorsTabState extends State<DoctorsTab> {
  ...
  Future<List<Doctor>> _load() async { try { return (await api.getDoctors()).map(Doctor.fromJson).toList(); } catch (_) { return fallbackDoctors; } }
...
class _BookingPageState extends State<BookingPage> {
  ...
  Future<void> confirm() async { final doctor = widget.doctor ?? fallbackDoctors.first; if (selectedSlot == null || saving) return; setState(() => saving = true); try { final dt = appointmentDateTime(); final saved = await api.createAppointment(patientName: patientName, phone: patientPhone, doctorId: doctor.id, appointmentAt: dt, mode: mode); appointments.value = [...appointments.value, Appointment(id: saved['id'] as int?, doctor: doctor, date: dt, slot: selectedSlot!, mode: mode, status: '${saved['status'] ?? 'confirmed'}')]; if (!mounted) return; await showDialog(context: context, builder: (_) => AlertDialog(title: const Text('Appointment Confirmed'), content: Text('${doctor.name}\n${selectedDate.day}/${selectedDate.month}/${selectedDate.year} • $selectedSlot\nMode: $mode'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Done'))])); } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Booking failed: $e'))); } finally { if (mounted) setState(() => saving = false); } }
  @override Widget build(BuildContext c) => SimplePage(title: 'Book Appointment', children: [Text(widget.doctor?.name ?? 'Select your doctor', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), if (widget.doctor != null) Text(widget.doctor!.specialty, style: const TextStyle(color: Colors.black54)), OutlinedButton.icon(onPressed: chooseDate, icon: const Icon(Icons.calendar_month), label: Text('${selectedDate.day}/${selectedDate.month}/${selectedDate.year}')), const Text('Available time slots', style: TextStyle(fontWeight: FontWeight.bold)), Wrap(spacing: 8, runSpacing: 8, children: slots.map((slot) => ChoiceChip(label: Text(slot), selected: selectedSlot == slot, onSelected: (_) => setState(() => selectedSlot = slot))).toList()), const SizedBox(height: 16), DropdownButtonFormField<String>(initialValue: mode, decoration: const InputDecoration(labelText: 'Consultation mode', border: OutlineInputBorder()), items: ['Video', 'In-clinic'].map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(), onChanged: (x) => setState(() => mode = x!)), const SizedBox(height: 16), SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: selectedSlot == null || saving ? null : confirm, icon: saving ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.check_circle_outline), label: Text(saving ? 'Booking...' : 'Confirm Appointment'))]);
}
...
class RecordsTab extends StatefulWidget { const RecordsTab({super.key}); @override State<RecordsTab> createState() => _RecordsTabState(); }
class _RecordsTabState extends State<RecordsTab> {
  bool loading = true; String? error; List<Map<String, dynamic>> prescriptions = []; List<Map<String, dynamic>> records = [];
  @override void initState() { super.initState(); load(); }
  Future<void> load() async { setState(() { loading = true; error = null; }); try { final results = await Future.wait([api.getPrescriptions(patientPhone), api.getHealthRecords(patientPhone)]); if (!mounted) return; setState(() { prescriptions = results[0]; records = results[1]; }); } catch (e) { if (mounted) setState(() => error = '$e'); } finally { if (mounted) setState(() => loading = false); } }
  @override Widget build(BuildContext c) => Scaffold(appBar: AppBar(title: const Text('Health Records'), actions: [IconButton(onPressed: load, icon: const Icon(Icons.refresh))]), body: RefreshIndicator(onRefresh: load, child: ListView(padding: const EdgeInsets.all(16), children: [if (loading) const Padding(padding: EdgeInsets.all(40), child: Center(child: CircularProgressIndicator())), if (!loading && error != null) Card(child: ListTile(leading: const Icon(Icons.error_outline), title: const Text('Unable to load health records'), subtitle: Text(error!), trailing: IconButton(onPressed: load, icon: const Icon(Icons.refresh)))), if (!loading && error == null) ...[
    const Text('Prescriptions', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 8),
    if (prescriptions.isEmpty) const Card(child: ListTile(leading: Icon(Icons.medication_outlined), title: Text('No prescriptions yet'), subtitle: Text('Your digital prescriptions will appear here.'))),
    ...prescriptions.map((p) => Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.medication_outlined)), title: Text('${p['diagnosis'] ?? 'Prescription'}'), subtitle: Text('${p['doctor_name'] ?? 'Doctor'} • ${p['specialty'] ?? ''}'), trailing: const Icon(Icons.chevron_right), onTap: () => showPrescription(c, p)))),
...
void showPrescription(BuildContext c, Map<String, dynamic> p) => showModalBottomSheet(context: c, isScrollControlled: true, builder: (_) => SafeArea(child: Padding(padding: const EdgeInsets.all(20), child: ListView(shrinkWrap: true, children: [Text('Digital Prescription', style: Theme.of(c).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text('Doctor: ${p['doctor_name'] ?? 'Doctor'}'), Text('Specialty: ${p['specialty'] ?? '—'}'), Text('Diagnosis: ${p['diagnosis'] ?? '—'}'), if ('${p['notes'] ?? ''}'.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: Text('Notes: ${p['notes']}')), const SizedBox(height: 16), const Text('Medicines', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)), const SizedBox(height: 8), ...medicineItems(p['medicines']), const SizedBox(height: 12), Text('Prescribed: ${p['prescribed_at'] ?? '—'}', style: const TextStyle(color: Colors.black54))])));
List<Widget> medicineItems(dynamic value) { if (value is! List) return [const Text('Medicine details unavailable')]; return value.map<Widget>((m) { final x = Map<String, dynamic>.from(m as Map); return Card(child: ListTile(title: Text('${x['medicine_name'] ?? 'Medicine'}'), subtitle: Text('${x['dosage'] ?? ''} • ${x['frequency'] ?? ''} • ${x['duration'] ?? ''}${'${x['instructions'] ?? ''}'.isNotEmpty ? '\n${x['instructions']}' : ''}'))); }).toList(); }
void showRecord(BuildContext c, Map<String, dynamic> r) => showModalBottomSheet(context: c, builder: (_) => SafeArea(child: Padding(padding: const EdgeInsets.all(20), child: ListView(shrinkWrap: true, children: [Text('${r['title'] ?? 'Medical Record'}', style: Theme.of(c).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)), const SizedBox(height: 12), Text('Type: ${r['record_type'] ?? '—'}'), Text('Date: ${r['record_date'] ?? '—'}'), Text('Source: ${r['source'] ?? '—'}'), if ('${r['description'] ?? ''}'.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 12), child: Text('${r['description']}'))])));
