...
const server=http.createServer((req,res)=>{
  ...
  if(req.url==='/api/auth/request-otp' && req.method==='POST') return parseBody(req,(err,data)=>{if(err||!data.phone)return send(res,400,{error:'phone is required'});const otp=issueOtp(data.phone);send(res,200,{message:'OTP generated for development',devOtp:otp});});
