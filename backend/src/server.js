...
const isValidPhone=(value)=>/^[0-9+() -]{7,20}$/.test(String(value).trim());
const isValidAppointment=(data)=>isNonEmptyString(data.patientName)&&isValidPhone(data.phone)&&Number.isInteger(Number(data.doctorId))&&Number(data.doctorId)>0&&isNonEmptyString(data.appointmentAt)&&(!data.mode||data.mode==='Video'||data.mode==='In-clinic');
const server=http.createServer(async(req,res)=>{
  ...
  if(req.url==='/api/auth/request-otp'&&req.method==='POST')return parseBody(req,(err,data)=>{if(err||!isValidPhone(data.phone))return send(res,400,{error:'valid phone is required'});send(res,200,{message:'OTP generated for development',devOtp:issueOtp(data.phone)});});
  if(req.url==='/api/auth/verify-otp'&&req.method==='POST')return parseBody(req,(err,data)=>{if(err||!isValidPhone(data.phone)||!data.otp)return send(res,400,{error:'valid phone and otp are required'});if(!verifyOtp(data.phone,String(data.otp)))return send(res,401,{error:'Invalid or expired OTP'});send(res,200,{token:createDevToken(data.phone),user:{phone:data.phone}});});
  if(req.url==='/api/appointments'&&req.method==='GET')return send(res,200,appointments);
  if(req.url==='/api/appointments'&&req.method==='POST')return parseBody(req,async(err,data)=>{if(err)return send(res,400,{error:'Invalid JSON'});if(!isValidAppointment(data))return send(res,422,{error:'patientName, valid phone, doctorId and appointmentAt are required; mode must be Video or In-clinic'});try{const saved=await createAppointment({...data,doctorId:Number(data.doctorId)});if(saved)return send(res,201,saved);}catch(e){return send(res,503,{error:'Unable to create appointment'});}const appointment={id:appointments.length+1,status:'confirmed',mode:data.mode||'Video',...data,doctorId:Number(data.doctorId)};appointments.push(appointment);send(res,201,appointment);});
